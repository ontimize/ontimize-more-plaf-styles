package com.ontimize.plaf.style.jar;
...
public class JarUtil {
	...
	public static final String TITLE_VALUE="OntimizePLAF LightBlue";
	...
	public static String getManifest(Component d) throws Exception {
		...
		if (manifest != null) {
			try {
				...
				String ontimizePLAF_version = (String)getAttribute("Ontimize-plaf-version-number", manifest);
				URL urlHtml = JarUtil.class.getClassLoader().getResource("com/ontimize/plaf/style/jar/template.html");
				if (urlHtml != null) {
					...
					if(ontimizePLAF_version!=null){
						sOutput = sOutput.replaceAll("%ontimize-plaf-version%", ontimizePLAF_version);
					}else{
						sOutput = sOutput.replaceAll("%ontimize-plaf-version%", " -- ");
					}
					return sOutput;
				}
			} catch (IOException e) {
	...
	public static String ontimizeVersion() throws Exception {
		...
		try {
			...
			String attr = getAttribute("Ontimize-plaf-version-number", manifest);
	...
	public static class InformationDialog extends JFrame {
		...
		public InformationDialog(boolean hideFrame) {
			...
			setTitle("Imatia");
			URL url = JarUtil.class.getClassLoader().getResource("com/ontimize/plaf/style/jar/iconimatia.gif");
			...
			url = JarUtil.class.getClassLoader().getResource("com/ontimize/plaf/style/jar/logoontimize.jpg");
